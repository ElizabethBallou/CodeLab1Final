...
public class GridManager : MonoBehaviour
{
    ...
    public TeacupList teacupList;
    private int TeacupIndex;
    ...
    void Start()
    {
        TeacupIndex = teacupList.TeacupIndex;
...
//        grid[0] = Instantiate(Resources.Load<GameObject>("Prefabs/Cube"));
        ...
        for (int x = 0; x < grid.Length; x++) //for loop to go through all the columns 
        {
            ...
            {
                grid[x][y] = Instantiate(Resources.Load<GameObject>("Prefabs/teacuporiginal"));
               grid[x][y].transform.position = new Vector3(x *1.1f, y * 1.1f, 0); //use the column, row to position the cube
            }
        }
    }
